...
//					+ plainText.substring(8, 16).toUpperCase() + "\n");
		...
		String DESDecrypt(String plainText, String key) {
			int i;
			// get round keys
			String keys[] = getKeys(key);
...
//					+ plainText.substring(8, 16).toUpperCase() + "\n");
		...
		String encryptText(String text, String key) {
			...
			text = StringToHex(text.trim());
		    key = StringToHex(key.trim());
			
			String eText = "";
			int noOfBlocks;
			if ((text.length()) % 16 == 0) {
				noOfBlocks = (text.length()) / 16;
				for (int i = 0; i < noOfBlocks * 16; i += 16) {
					eText += DESEncrypt(text.substring(i, i + 16), key);
				}
			} else {
				noOfBlocks = text.length() / 16;
				for (int i = 0; i < noOfBlocks * 16; i += 16) {
				...
				int leftChars = text.length() - (noOfBlocks * 16);
				String lastBlock = text.substring(noOfBlocks * 16, noOfBlocks * 16 + leftChars);
				int noOfSpaces = 16 - leftChars;
		...
		String decryptText(String cText, String key) {
			...
			String text = "";
			int noOfBlocks = cText.length() / 16;
			for (int i = 0; i < noOfBlocks * 16; i += 16) {
				text += DESDecrypt(cText.substring(i, i + 16), key);
			}
			return HexToString(text).trim();
		}
